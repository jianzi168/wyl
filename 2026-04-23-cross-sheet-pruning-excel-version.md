...
**跨表链路**：
- B2（表A）→ D7（表A）→ H3（表B，跨表引用表A!D7）→ F3（表B）→ I7（表C，跨表引用表B!F3）
- **链路长度**：4 步（跨表 2 层：表A → 表B → 表C）

**剪支关键**：C4 不在 B2 的传播路径上（B2 的值变化只沿着依赖边传播，C4 依赖 D5 和 B8，与 B2 无依赖关系），因此 C4 被剪支，不重新计算。
